...
class CharterNote extends UISprite implements ICharterSelectable {
	...
	private function drawTypeText() {
		...
		if (FlxG.keys.pressed.SHIFT) {
			if (typeText.text != Charter.instance.noteTypes[type-1]) typeText.text = Charter.instance.noteTypes[type-1];
		} else {
			if (typeText.text != Std.string(type)) typeText.text = Std.string(type);
		}	
