...
@:unreflective
class PlayData {
	//hardcoded encrypted values to prevent cheating with cheat engine type programs (hopefullysdfhujiashdfjkl)
	public static final marvHitWindow:EncryptedFloat = new EncryptedFloat(25.0);
	public static final sickHitWindow:EncryptedFloat = new EncryptedFloat(45.0);
	public static final goodHitWindow:EncryptedFloat = new EncryptedFloat(90.0);
	public static final badHitWindow:EncryptedFloat = new EncryptedFloat(135.0);

	public static final marvScore:EncryptedInt = new EncryptedInt(350);
	public static final sickScore:EncryptedInt = new EncryptedInt(300);
	public static final goodScore:EncryptedInt = new EncryptedInt(200);
	public static final badScore:EncryptedInt = new EncryptedInt(100);
	public static final shitScore:EncryptedInt = new EncryptedInt(-100);

	public static final marvAcc:EncryptedFloat = new EncryptedFloat(1.0);
	public static final sickAcc:EncryptedFloat = new EncryptedFloat(1.0);
	public static final goodAcc:EncryptedFloat = new EncryptedFloat(0.75);
	public static final badAcc:EncryptedFloat = new EncryptedFloat(0.2);
	public static final shitAcc:EncryptedFloat = new EncryptedFloat(0.05);

	public static final missScore:EncryptedInt = new EncryptedInt(-500);
}
...
class PlayState extends MusicBeatState
{
	 ...
	 */
	@:unreflective
	public var songScore:EncryptedInt = new EncryptedInt(0);
	/**
	 * The player's amount of misses.
	 */
	@:unreflective
	public var misses:EncryptedInt = new EncryptedInt(0);
	/**
	 ...
	 */
	 @:unreflective
	public var accuracyPressedNotes:EncryptedFloat = new EncryptedFloat(0.0);
	/**
	 * The total accuracy amount.
	 */
	 @:unreflective
	public var totalAccuracyAmount:EncryptedFloat = new EncryptedFloat(0.0);
	...
	private function get_accuracy():Float {
		if (accuracyPressedNotes.value <= 0) return -1;
		return totalAccuracyAmount.value / accuracyPressedNotes.value;
	}
	private function set_accuracy(v:Float):Float {
		if (accuracyPressedNotes.value <= 0)
			accuracyPressedNotes.value = 1;
		return totalAccuracyAmount.value = v * accuracyPressedNotes.value;
	}
	/**
	 * All combo ratings.
	 */
	...
	function updateRatingStuff() {
		scoreTxt.text = 'Score:' + songScore.value;
		missesTxt.text = '${comboBreaks ? "Combo Breaks" : "Misses"}:' + misses.value;
	...
	public function endSong():Void
	{
		...
		if (validScore && !chartingMode && !coopMode)
		{
			#if !switch
			FunkinSave.setSongHighscore(SONG.meta.name, difficulty, {
				score: songScore.value,
				misses: misses.value,
				accuracy: accuracy,
				hits: [],
			...
			Leaderboards.addHighScore(
				SONG.meta.name.toLowerCase(), 
				difficulty.toLowerCase(), 
				Script.staticVariables.get("songSpeed"), 
				opponentMode, 
				CoolUtil.quantize(accuracy * 100, 100), 
				songScore.value, 
				misses.value
			);
		}
	...
	public function noteMiss(strumLine:StrumLine, note:Note, ?direction:Int, ?player:Int):Void
	{
		...
		songScore.value += PlayData.missScore.value;
		misses.value += event.misses;
		...
		if (event.accuracy != null) {
			accuracyPressedNotes.value += 1;
			totalAccuracyAmount.value += event.accuracy;
	...
	public function goodNoteHit(strumLine:StrumLine, note:Note):Void
	{
		...
		if (noteDiff < PlayData.marvHitWindow.value) {
			daRating = "sick";
			score = PlayData.marvScore.value;
			accuracy = PlayData.marvAcc.value;
		} else if (noteDiff < PlayData.sickHitWindow.value) {
			daRating = "sick";
			score = PlayData.sickScore.value;
			accuracy = PlayData.sickAcc.value;
		} else if (noteDiff < PlayData.goodHitWindow.value) {
			daRating = "good";
			score = PlayData.goodScore.value;
			accuracy = PlayData.goodAcc.value;
		} else if (noteDiff < PlayData.badHitWindow.value) {
			daRating = "bad";
			score = PlayData.badScore.value;
			accuracy = PlayData.badAcc.value;
		} else {
			daRating = "shit";
			score = PlayData.shitScore.value;
			accuracy = PlayData.shitAcc.value;
		}
		...
		if (!event.cancelled) {
			if (!note.isSustainNote) {
				if (event.countScore) songScore.value += event.score;
				if (event.accuracy != null) {
					accuracyPressedNotes.value += 1;
					totalAccuracyAmount.value += event.accuracy;
					updateRating();
