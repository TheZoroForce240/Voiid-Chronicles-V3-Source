...
class StrumLine extends FlxTypedGroup<Strum> {
	...
	public function updateNote(daNote:Note) {
		...
		if (__updateNote_event.__updateHitWindow) {
			daNote.canBeHit = (daNote.strumTime > __updateNote_songPos - (166.0 * daNote.latePressWindow)
				&& daNote.strumTime < __updateNote_songPos + (166.0 * daNote.earlyPressWindow));

			if (daNote.strumTime < __updateNote_songPos - 166.0 && !daNote.wasGoodHit)
				daNote.tooLate = true;
		}
