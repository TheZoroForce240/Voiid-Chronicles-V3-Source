...
import voiid.EncryptedValue.EncryptedString;
...
class FlxGameJolt
{
	 ...
	 */
	 @:unreflective
	static var _privateKey:EncryptedString = new EncryptedString("");
	...
	{
		if (_gameID != 0 && _privateKey.value != "")
			return;
...
//		trace(_gameID);
	...
	static function encryptURL(Url:String):String
	{
		...
		{
			return Sha1.encode(Url + _privateKey.value);
		}
		else
		{
			return Md5.encode(Url + _privateKey.value);
		}
	}
	...
	static function get_gameInit():Bool
	{
		if (_gameID == 0 || _privateKey.value == "")
		{
			...
			FlxG.log.warn("FlxGameJolt: You must run init() before you can do this. Game ID is "
				+ _gameID
				+ " and the key is "
				+ "who thought this was a good idea to have the private key print here you dumbass"
				+ ".");
			#end
			return false;
		}
