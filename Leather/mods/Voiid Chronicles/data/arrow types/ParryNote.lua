...
function createPost()
	...
	setActorAntialiasing(true, 'flamePunch')
	setActorStageShader('flamePunch')
	setActorAlpha(0, 'flamePunch')

	initShader("parryRtx", "RTXEffect")

	makeAnimatedSprite('parryAttack', 'characters/WIIK_3_THROW', getMattCurrentX()-420, getMattCurrentY()-440)
	addActorAnimation('parryAttack', 'Matt Attack FistThrow', 'Matt Attack FistThrow', 24, false)
	setActorAntialiasing(true, 'parryAttack')
	--setActorProperty('parryAttack', 'shader', getActorProperty('dadCharacter0', 'shader'))
	playActorAnimation('parryAttack', 'Matt Attack FistThrow', true)
	setActorAlpha(0, 'parryAttack')
	setActorRTXShader('parryAttack', "parryRtx")
	--setActorShader('parryAttack', "echo")
	...
	if #parries > 0 then 
        if parries[1][1]-parryAnimTime < songPos then
            if not parries[1][2] then 
				...
				playActorAnimation('parryAttack', 'Matt Attack FistThrow', true)
				setActorAlpha(1, 'parryAttack')
				setActorX(getMattCurrentX()-420, 'parryAttack')
				setActorY(getMattCurrentY()-440, 'parryAttack')

				if player2 == 'Wiik3VoiidMatt' then 
					setActorAlpha(0.5, 'dadCharacter0')
				else 
					setActorAlpha(0.5, 'parryAttack')
				end

                --trace('play anim')
                tweenActorProperty('parryAttack', 'x', (getMattCurrentX()-420)-450, parryAnimTime*0.001, 'cubeOut')
                parries[1][2] = true
            end
            ...
            if parries[1][1]-100 < songPos and songPos < parries[1][1]+(punchLateHitTiming*1.5) then
                ...
                if not parries[1][3] then
                    ...
                    setActorY(getActorY('parryAttack')+40, 'flamePunch')
                    setActorX(getActorX('parryAttack')-150, 'flamePunch')
                    tweenActorProperty('parryAttack', 'x', getMattCurrentX()-420, parryAnimTime*0.001, 'cubeIn')
...
function parryRemove()
	...
	setActorAlpha(0, 'parryAttack')
	if player2 == 'Wiik3VoiidMatt' then 
		setActorAlpha(1.0, 'dadCharacter0')
	end
