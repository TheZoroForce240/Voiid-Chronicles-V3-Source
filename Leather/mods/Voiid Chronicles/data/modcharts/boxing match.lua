...
function createPost()
	defaultMattY = getActorY('dadCharacter0')
	if modcharts then 
		for i = 0, 7 do 
			setActorY(2000, i)
...
function update(elapsed)
	...
	if doingPowerup and not getActorProperty('dadCharacter0', 'hasPowerup') then 
		...
		setActorProperty('dadCharacter0', 'y', defaultMattY-100 + math.sin(floatTime)*50)
	end
...
function beatHit()
	...
	if curBeat == 360 or curBeat == 1000 then 
		...
		if not getActorProperty('dadCharacter0', 'hasPowerup') then 
			tweenActorProperty('dadCharacter0', 'y', defaultMattY - 100, (crochet/1000)*16, 'cubeIn')
		end
	...
	elseif curBeat == 400 then 
		doingPowerup = false
		floatTime = 0
		tweenActorProperty('dadCharacter0', 'y', defaultMattY, (crochet/1000)*8, 'sineOut')
		playCharacterAnimation('dad', 'destrans', true)
	end
end