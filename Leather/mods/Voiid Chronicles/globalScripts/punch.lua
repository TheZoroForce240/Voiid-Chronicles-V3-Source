    ...
    if songHasPunches then 
        ...
        for i = 1,3 do 
            ...
            if not middlescroll then 
                setActorX(getActorX('punch'..i)+320, 'punch'..i)
            end
            setActorLayer('punch'..i, 1)
        end
...
function centerShit()
    local pos = getActorX(keyCount)
    local width = (getActorX((keyCount+playerKeyCount)-1)+getActorWidth((keyCount+playerKeyCount)-1))-pos
    if centerDodges then 
        pos = 0
        width = 1280
    end
    for i = 1,3 do 
        setActorX(pos + (width*0.5) - (getActorWidth('punch'..i)*0.5), 'punch'..i)
    end
    setActorX(pos + (width*0.5) - (getActorWidth('msText')*0.5), 'msText')
    setActorX(pos + (width*0.5) - (getActorWidth('punchesLeft')*0.5), 'punchesLeft')
end
...
function update(elapsed)
    ...
    if #punches > 0 then 
        ...
        if punches[1][1]-800 < songPos then
            punchAlphas[punches[1][2]] = 1 --show before
        end
