...
class Leaderboards
{
    ...
    public static function getHighscoreSaveString(songname:String, songdiff:String, speed:Float, opponent:Bool)
    {
        ...
        {
            case "sporting": 
                saveStr = "new-new-"+saveStr; 
            case "ruckus" | "mattpurgation" | "warm-up" | "target-practice" | "light-it-up" | "alter-ego" | "blastout" | "fisticuffs" | "king-hit" | "burnout" | "tko" | "flaming-glove" | "mat": 
                saveStr = "new-"+saveStr; 
        }
    ...
    public static function removeTopScore(songname:String, songdiff:String, speed:Float, opponent:Bool)
    {
        songname = songname.replace(" ", "-");
        songdiff = songdiff.replace(" ", "-");
            ...
            FlxGameJolt.fetchData(saveStr, false, function(mapThing:Map<String,String>)
            {
                if (mapThing != null)
                {
                    if (mapThing.get('success') == 'true' && mapThing.exists('data'))
                    {
                        ...
                        trace('delete ' + scoreList.scores[0]);

                        scoreList.scores.remove(scoreList.scores[0]);
                        ...
                        {
                            //trace(mapThing);
                        }, "addHighscoreasdasdasdasd");
                    }
                }
            }, "addHighscoreasdasdasd");
        }
    }
}