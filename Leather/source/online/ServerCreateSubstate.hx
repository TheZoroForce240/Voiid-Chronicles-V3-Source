...
class ServerCreateSubstate extends MusicBeatSubstate
{
                ...
                FlxGameJolt.fetchData(getServerName("P0"), false, function(mapThing:Map<String,String>)
                {
                    ...
                    {        
                        if (mapThing.get('success') == 'true' && mapThing.exists('data'))
                        {
                            ...
                            if (Multiplayer.checkTimestampsForDisconnect(client.lastTimestamp, Multiplayer.currentTimestamp) || !client.playerConnected) //in case theres an old disconnected server
                            {
                            ...
                            FlxGameJolt.fetchData(getServerName("P1"), false, function(mapThing2:Map<String,String>)
                            {
                                ...
                                {                
                                    if (mapThing2.get('success') == 'true' && mapThing2.exists('data'))
                                    {
                                        ...
                                        var hostClient = Multiplayer.parseClientData(mapThing.get('data'));
                                        var opponentClient = Multiplayer.parseClientData(mapThing2.get('data'));
                                        if (Multiplayer.checkTimestampsForDisconnect(hostClient.lastTimestamp, Multiplayer.currentTimestamp) || !hostClient.playerConnected) //in case theres an old disconnected server
                                        {
                                            statusText.text = 'Server does not exist!!!'; //it does exist its just disconnected
                                        }
                                        else if (Multiplayer.checkTimestampsForDisconnect(opponentClient.lastTimestamp, Multiplayer.currentTimestamp) || !opponentClient.playerConnected) //opponent is disconnected, should be joinable????
                                        {
            ...
            FlxGameJolt.fetchData(getServerName("P0"), false, function(mapThing:Map<String,String>)
            {
                ...
                {    
                    if (mapThing.get('success') == 'true' && mapThing.exists('data'))
                    {
                        var asString = mapThing.get('data');
                        var client = Multiplayer.parseClientData(asString);
                        if (Multiplayer.checkTimestampsForDisconnect(client.lastTimestamp, Multiplayer.currentTimestamp) || !client.playerConnected)
                        {
	...
	override function update(elapsed:Float)
    {
        ...
        if (controls.BACK && !serverIDInput.hasFocus && !Multiplayer.active)
        {
