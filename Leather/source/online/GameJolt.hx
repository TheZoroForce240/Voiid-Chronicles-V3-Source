...
class GameJolt
{
    ...
    public static function initStuffs():String
    {
        ...
        FlxGameJolt.init(gameID, privateKey, true, FlxG.save.data.gameJoltUserName, FlxG.save.data.gameJoltUserToken, function(didLoginProperly:Bool) 
        {
            ...
            if (loggedIn)
            {
                trace('logged in');
                setupSession();
                Main.popupManager.addPopup(new MessagePopup(2.5, 300, 60, 'Signed into GameJolt as ' + FlxGameJolt.getUserName()));
    ...
    static var loadedFriends:Bool = false;
    public static var friendsData:Map<String, Map<String,String>> = [];
    public static function loadFriends()
    {
        loadedFriends = true;
        friendsData.clear();
        FlxGameJolt.fetchFriends(function(map:Map<String,String>)
        {
            trace(map);
            var friendsList:Array<Int> = [];
            for (id => val in map)
            {
                if (id.contains("friend_id"))
                {
                    friendsList.push(Std.parseInt(val));
                    trace(val);
                }
            }
            trace(friendsList);
            if (friendsList.length > 0)
            {
                ...
                load = function()
                {
                    if (friendsList.length > 0)
                    {
                        FlxGameJolt.fetchUser(friendsList[0], null, null, function(userMap:Map<String,String>)
                        {
                            trace(userMap);

                            var copyMap = new Map<String,String>();
                            for (i => v in userMap)
                                copyMap.set(i, v); //copy over

                            friendsData.set(Std.string(friendsList[0]), copyMap);
                            friendsList.remove(friendsList[0]);
                            load(); //try load next friend
                        });
                    }
                    else 
                    {
