...
class TitleState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (keyIdx < keyShit.length)
		{
			if (FlxG.keys.checkStatus(FlxKey.fromString(keyShit[keyIdx]), JUST_PRESSED))
			{
				keyIdx++;
				if (keyIdx >= keyShit.length)
				{
	...
	function glungus()
	{
		...
		var fileName:String = #if sys Sys.getCwd() + PolymodAssets.getPath(Paths.video(name, "mp4")) #else Paths.video(name, "mp4") #end;
