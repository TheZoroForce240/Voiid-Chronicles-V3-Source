...
class ModchartUtilities
{
    ...
    function new(?path:Null<String>)
    {
        ...
        var classBlackList = ["ChartChecker", "GameJoltStuff", "Popup", "FlxGameJolt", "Options", "AwardManager", "Multiplayer", "GameJolt", "Leaderboards", "ChatManager"];
        ...
        lua_Cameras.set("characters", {cam: PlayState.instance.camCharacters, shaders: [], shaderNames: []});
        lua_Cameras.set("back", {cam: PlayState.instance.camBack, shaders: [], shaderNames: []});
        ...
        setVar("centerDodges", utilities.Options.getData("centerDodges"));
        ...
        setVar("songMultiplier", PlayState.songMultiplier);
        setVar("inverseSongMultiplier", 1/PlayState.songMultiplier);
        ...
        setVar("resWidth", Capabilities.screenResolutionX);
        setVar("resHeight", Capabilities.screenResolutionY);
        ...
        Lua_helper.add_callback(lua,"addCharacterToMap", function(m:String, character:String) {
            ...
            if ((map == PlayState.instance.bfMap || map == PlayState.instance.dadMap) && cfg.characters == null)
            {
                var skin = SkinManager.getSkin(map == PlayState.instance.bfMap, isBoxing, false);
        ...
        Lua_helper.add_callback(lua,"makeSpriteCopy", function(id:String, targetID:String) {
            ...
            if(!lua_Sprites.exists(id) && actor != null)
            {
                ...
                if (actor is ReflectedSprite)
                {
                    var a:ReflectedSprite = cast actor;
                    Sprite.drawFlipped = a.drawFlipped; //copy flip for characters
                }
        ...
        Lua_helper.add_callback(lua, "makeCameraSpriteClone", function(name:String, cam:String)
        {
        ...
        Lua_helper.add_callback(lua,'changeHealthRange', function (minHealth:Float, maxHealth:Float) {
            ...
            @:privateAccess
            {
                ...
                if (PlayState.instance.gameHUD.backBar != null)
                    PlayState.instance.gameHUD.backBar.setRange(minHealth, maxHealth);
            }
        });
        ...
        Lua_helper.add_callback(lua,"setRenderedNoteZ", function(z:Float, id:Int) {
            //PlayState.instance.notes.members[id].modifiedByLua = true;
            PlayState.instance.notes.members[id].z = z;
        });
        ...
        Lua_helper.add_callback(lua,"setCharacterExtraSuffix", function(id:String, suff:String) {
            if(getCharacterByName(id) != null)
                {
                    var character = getCharacterByName(id);
                    if (character.otherCharacters != null && character.otherCharacters.length > 0)
                    {
                        character.otherCharacters[0].extraSuffix = suff;
                        return;
                    }
            ...
            if(getActorByName(id) != null)
            {
                getActorByName(id).extraSuffix = suff;
            }
        });
        ...
        Lua_helper.add_callback(lua,"tweenWindowProperty",function(prop:String, value:Float, easeStr:String, time:Float) {
        ...
        Lua_helper.add_callback(lua,"tweenScrollSpeed",function(speed:Float, time:Float, easeStr:String) {
            PlayState.instance.tweenScrollSpeed(speed, time, getEase(easeStr));
        });
        ...
        Lua_helper.add_callback(lua,"setActorStageShader", function(actorStr:String) {
            ...
            {
                @:privateAccess
                actor.shader = PlayState.instance.stage.colorSwap.shader;
        ...
        Lua_helper.add_callback(lua,"setActorRTXShader", function(actorStr:String, shaderName:String) {
            if (!utilities.Options.getData("shaders"))
                return;
            ...
            if(actor != null && shad != null)
            {
               ...
               // {
                    ...
                    @:privateAccess
                    var stage_Data = PlayState.instance.stage.stage_Data;
        ...
        Lua_helper.add_callback(lua,"setShaderTargetCam", function(shaderName:String, prop:String, camStr:String) {
            if (!utilities.Options.getData("shaders"))
                return;
            var shad = lua_Shaders.get(shaderName);
            ...
            if(shad != null)
            {
                Reflect.setProperty(shad, prop, getCameraByName(camStr).cam);
            }
        });
        ...
        Lua_helper.add_callback(lua,"tweenActorRTXColor", function(character:String, prop:String, value:String, time:Float, easeStr:String = "linear") {
            if (!utilities.Options.getData("shaders"))
                return;
            ...
            if (char != null)
            {
                ...
                PlayState.instance.tweenManager.color(null, time, startVal, endVal, {onUpdate: function(tween:FlxTween){
        ...
        Lua_helper.add_callback(lua,"getActorRTXColor", function(character:String, prop:String) {
            var char:Character = getCharacterByName(character);
            if (char != null)
            {
        ...
        Lua_helper.add_callback(lua,"setActorRTXProperty", function(character:String, prop:String, value:Dynamic) {
            if (!utilities.Options.getData("shaders"))
                return;
            ...
            if (char != null)
            {
                ...
                {
                    for (c in char.otherCharacters) //set for every character
                    {
        ...
        Lua_helper.add_callback(lua,"tweenActorRTXProperty", function(character:String, prop:String, value:Dynamic, time:Float, easeStr:String = "linear") {
            if (!utilities.Options.getData("shaders"))
                return;
        ...
        Lua_helper.add_callback(lua,"getActorProperty", function(id:String, prop:String) {
            var actor = getActorByName(id);
            if(actor != null && Reflect.getProperty(actor, prop) != null)
            {
                return Reflect.getProperty(actor, prop);
            }
            return null;
        });
        ...
        Lua_helper.add_callback(lua,"getStageColorSwap", function(prop:String) {
            @:privateAccess
            var actor = PlayState.instance.stage.colorSwap;
        ...
        Lua_helper.add_callback(lua,"setGameFilters", function(shaderNames:String) {
            if (!utilities.Options.getData("shaders"))
                return;


            var shads = shaderNames.split(",");

            var shadArr:Array<BitmapFilter> = [];

            for (s in shads)
            {
                var shad = lua_Shaders.get(s);
                if (shad != null)
                    shadArr.push(new ShaderFilter(Reflect.getProperty(shad, 'shader')));
            }

              
            FlxG.game.setFilters(shadArr);
        });
        ...
        Lua_helper.add_callback(lua,"printLuaObjectNames", function() {
            trace(lua_Sprites);
        });

        Lua_helper.add_callback(lua,"setGlobalVar", function(name:String, value:Dynamic) {
            PlayState.instance.setLuaVar(name, value);
        });
        Lua_helper.add_callback(lua,"getGlobalVar", function(name:String, type:String) {
            PlayState.instance.getLuaVar(name, type);
        });

        Lua_helper.add_callback(lua,"showBinds", function() {
            var strums = PlayState.characterPlayingAs == 0 ? PlayState.playerStrums : PlayState.enemyStrums;
            //if (!utilities.Options.getData("extraKeyReminders") || strums.length == 4)
                //return;
            ...
            for (i in 0...strums.members.length)
            {
                ...
                @:privateAccess
                var keyStr = PlayState.instance.binds[i];
                if (MusicBeatState.usingController) @:privateAccess
                    keyStr = NoteHandler.formatControllerBind(PlayState.instance.controllerBinds[i]);
                ...
                FlxTween.tween(keyThingLolShadow, {y: keyThingLolShadow.y + 10, alpha: 0}, 3, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i), onComplete: function(_){
                    PlayState.instance.remove(keyThingLolShadow);
                ...
                FlxTween.tween(keyThingLol, {y: keyThingLol.y + 10, alpha: 0}, 3, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i), onComplete: function(_){
                    PlayState.instance.remove(keyThingLol);
                    keyThingLol.kill();
                    keyThingLol.destroy();
                }});
        ...
        Lua_helper.add_callback(lua,"randomBool", function(p:Float) {
            return FlxG.random.bool(p);
        });
        Lua_helper.add_callback(lua,"randomFloat", function(min:Float, max:Float) {
            return FlxG.random.float(min, max);
        });

        Lua_helper.add_callback(lua,"getCharacterX", function(idx:Int) {
            @:privateAccess
            return PlayState.instance.stage.stage_Data.character_Positions[idx][0];
        });
        Lua_helper.add_callback(lua,"getCharacterY", function(idx:Int) {
            @:privateAccess
            return PlayState.instance.stage.stage_Data.character_Positions[idx][1];
        });

        Lua_helper.add_callback(lua,"getCharacterOffsetFromStageX", function(id:String) {
            if(getCharacterByName(id) != null)
            {
                var character = getCharacterByName(id);
                return character.offsetFromStage[0];
            }
            return 0;
        });
        Lua_helper.add_callback(lua,"getCharacterOffsetFromStageY", function(id:String) {
            if(getCharacterByName(id) != null)
            {
                var character = getCharacterByName(id);
                return character.offsetFromStage[1];
            }
            return 0;
        });

        Lua_helper.add_callback(lua,"getStageObjectList", function() {
    ...
    public function setupTheShitCuzPullRequestsSuck()
        {
            ...
            {
                lua_Sprites.set("dadCharacter0", PlayState.dad);
                lua_Characters.set("dadCharacter0", PlayState.dad);
                lua_Sprites.set("dad", PlayState.dad);
                lua_Characters.set("dad", PlayState.dad);
            }
            lua_Sprites.set("dadObject", PlayState.dad);
            lua_Characters.set("dadObject", PlayState.dad);
            //else 
           // {
            ...
            {
                lua_Sprites.set("bfCharacter0", PlayState.boyfriend);
                lua_Characters.set("bfCharacter0", PlayState.boyfriend);
                lua_Sprites.set("boyfriend", PlayState.boyfriend);
                lua_Characters.set("boyfriend", PlayState.boyfriend);
            }
            lua_Sprites.set("boyfriendObject", PlayState.boyfriend);
            lua_Characters.set("boyfriendObject", PlayState.boyfriend);
           // else 
            //{   
            ...
            {
                lua_Sprites.set("gfCharacter0", PlayState.gf);
                lua_Characters.set("gfCharacter0", PlayState.gf);
                lua_Sprites.set("girlfriend", PlayState.gf);
                lua_Characters.set("girlfriend", PlayState.gf);
            }
            //else 
            ...
            @:privateAccess
            {
                setVar("p1DefaultX", PlayState.instance.stage.player_1_Point.x);
                setVar("p1DefaultY", PlayState.instance.stage.player_1_Point.y);
                setVar("p2DefaultX", PlayState.instance.stage.player_2_Point.x);
                setVar("p2DefaultY", PlayState.instance.stage.player_2_Point.y);
                setVar("gfDefaultX", PlayState.instance.stage.gf_Point.x);
                setVar("gfDefaultY", PlayState.instance.stage.gf_Point.y);
            }
    ...
    function cameraFromString(cam:String):FlxCamera
    {
        ...
        if (camera == null)
        {
            switch(cam.toLowerCase())
            {
                ...
                case "back" : return PlayState.instance.camBack;
            }
