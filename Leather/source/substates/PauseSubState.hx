...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		{
			menus.get("default").insert(menus.get("default").length-1, new TimeSkipOption());
		}
		...
		if(accepted && !justPressedAcceptLol)
		{
			...
			switch(daSelected.toLowerCase())
			{
				case "time skip": 
					...
					pauseMusic.stop();
					close();
					PlayState.instance.skipTimeNextFrame = true;
				case "resume":
					pauseMusic.volume = 0;
					pauseMusic.stop();
					close();
				...
				case "bot":
					...
					warningAmountLols += 1;
				case 'mechanics' | "downscroll" | "middlescroll" | "use custom scrollspeed": 
	...
	override public function close()
	{
		...
		PlayState.instance.camGame.active = true; //unpause cameras
		PlayState.instance.camHUD.active = true;
		super.close();
	}
