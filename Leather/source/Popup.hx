...
class GlovePopup extends Popup
{
    var text:TextField;
    var image:Bitmap;
    var spriteImage:FlxSprite;
    override public function new(time:Float, w:Int = 300, h:Int = 120, gloveCol:String, gloveNum:Int)
    {
        super(time, w, h);
        text = new TextField();
        text.text = "+" + gloveNum;
        ...
        var imagePath = Paths.image("online/room/glove_"+gloveCol);
        //trace(imagePath);
        //spriteImage = new FlxSprite(0,0).loadGraphic(imagePath);
        //spriteImage.setGraphicSize(100, 100);
        //spriteImage.updateHitbox();

        var spr = FlxG.bitmap.add(imagePath, false, null);

        image = new Bitmap(spr.bitmap); //easy flixel to openfl shit
        addChild(image);
        image.smoothing = true;
        image.scaleY = h/spr.height;
        ...
        var ww = text.width + image.width + 10;

        image.x = (getScreenWidth() * 0.5) - (ww*0.5);
        text.x = ((getScreenWidth() * 0.5) - (ww*0.5)) + image.width+10;
    ...
    override public function update(deltaTime:Float)
    {
        ...
        //bg.x = (getScreenWidth() * 0.5) - (popupWidth * 0.5);

        var w = text.width + image.width;

        image.x = (getScreenWidth() * 0.5) - (w*0.5);
        text.x = ((getScreenWidth() * 0.5) - (w*0.5)) + image.width;
