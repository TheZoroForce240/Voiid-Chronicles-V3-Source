...
class Note extends perspective.FlxPerspectiveSprite
{
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?character:Int = 0, ?arrowType:String = "default", ?song:SwagSong, ?characters:Array<Int>, ?mustPress:Bool = false, ?inEditor:Bool = false)
	{
		...
		if (/*PlayState.SONG.song.toLowerCase() == 'galactic storm' && */arrow_Type == "Wiik3Punch" && PlayState.SONG.playerKeyCount >= 12)
			missDamage = 1; //no instakill 
	...
	override function update(elapsed:Float)
	{
		...
		//test += elapsed*5.0;
		//z = Math.sin(test+strumTime)*50.0;
