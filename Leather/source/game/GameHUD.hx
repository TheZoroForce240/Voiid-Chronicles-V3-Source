...
class GameHUD extends FlxTypedSpriteGroup<FlxSprite>
{
    ...
    public function createHUD(?uiSkin:String = "default", ?healthBarType:String = "default")
    {
        ...
        switch (healthBarType)
        {
            ...
            case "vanilla": 
                ...
                add(healthBar);
            case "greed": 
                var sprName:String = "GreedHealthBar" + (utilities.Options.getData("downscroll") ? "Down" : "Up");

                var xOff:Int = 24;
                var yOff:Int = 27;
                if (!utilities.Options.getData("downscroll")) //upscroll
                {
                    xOff = 28;
                    yOff = 58;
                }

                healthBarBG = new FlxSprite(0, healthBarPosY).loadGraphic(Paths.image('ui skins/' + uiSkin + '/other/'+sprName, 'shared'));
                healthBarBG.screenCenter(X);
                healthBarBG.scrollFactor.set();
                healthBarBG.pixelPerfectPosition = true;
                
                healthBar = new FlxBar(healthBarBG.x + xOff, healthBarBG.y + yOff, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 50), 23, this,
                    'visualHealth', minHealth, maxHealth);
                healthBar.scrollFactor.set();
                healthBar.createFilledBar(opponentColor, playerColor);
                healthBar.pixelPerfectPosition = true;
                ...
                healthBar.offset.y += 26;
                healthBarBG.offset.y += 26;
                iconYOffset = -20;
                if (!utilities.Options.getData("downscroll")) //upscroll
                {
                    healthBar.offset.y += 30;
                    healthBarBG.offset.y += 30;
                    iconYOffset += -30;
                }
        
                add(healthBarBG);
            case "rvip": 
                healthBarBG = new FlxSprite(0, healthBarPosY).loadGraphic(Paths.image('ui skins/' + uiSkin + '/other/newHealthBarRVIP', 'shared'));
                healthBarBG.screenCenter(X);
                healthBarBG.scrollFactor.set();
                healthBarBG.pixelPerfectPosition = true;

                backBar = new FlxBar(healthBarBG.x + 13, healthBarBG.y + 15, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 25), 23, this,
                    'lerpedHealth', minHealth, maxHealth);
                    backBar.scrollFactor.set();
                    backBar.createFilledBar(opponentColor, 0xFFFF7B00);
                    backBar.pixelPerfectPosition = true;
                add(backBar);
                backBar.offset.y += 10;
                ...
                healthBar = new FlxBar(healthBarBG.x + 13, healthBarBG.y + 15, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 25), 23, this,
                    'visualHealth', minHealth, maxHealth);
                healthBar.scrollFactor.set();
                healthBar.createFilledBar(0x00000000, playerColor);
                healthBar.pixelPerfectPosition = true;
                add(healthBar);
    ...
    override public function update(elapsed:Float)
    {
        ...
        lerpedHealth = FlxMath.lerp(lerpedHealth, visualHealth, elapsed);
    }
