...
class StageGroup extends FlxGroup
{
    ...
    public function setCharOffsets(?p1:Character, ?gf:Character, ?p2:Character):Void
    {
        ...
        p1.offsetFromStage = [player_1_Point.x-p1.x, player_1_Point.y-p1.y];
        gf.offsetFromStage = [gf_Point.x-gf.x, gf_Point.y-gf.y];
        p2.offsetFromStage = [player_2_Point.x-p2.x, player_2_Point.y-p2.y];
        ...
        if(p1.otherCharacters != null)
        {
            for(character in p1.otherCharacters)
            {
                ...
                character.offsetFromStage = [player_1_Point.x-character.x, player_1_Point.y-character.y];
        ...
        if(gf.otherCharacters != null)
        {
            for(character in gf.otherCharacters)
            {
                ...
                character.offsetFromStage = [gf_Point.x-character.x, gf_Point.y-character.y];
                //character.shader = colorSwap.shader;
            }
        }
        ...
        if(p2.otherCharacters != null)
        {
            for(character in p2.otherCharacters)
            {
                ...
                character.offsetFromStage = [player_2_Point.x-character.x, player_2_Point.y-character.y];
                //character.shader = colorSwap.shader;
            }
        }
    }
    ...
    function setCharacterRTX(c:Character)
    {
        if (stage_Data.rtxData != null)
        {
            ...
            if (!c.debugMode)
            {
                c.rtxShader.overlayColor = FlxColor.fromRGBFloat(overlay.redFloat, overlay.greenFloat, overlay.blueFloat, stage_Data.rtxData.overlayAlpha);
                c.rtxShader.satinColor = FlxColor.fromRGBFloat(satin.redFloat, satin.greenFloat, satin.blueFloat, stage_Data.rtxData.satinAlpha);
                c.rtxShader.innerShadowColor = FlxColor.fromRGBFloat(inner.redFloat, inner.greenFloat, inner.blueFloat, stage_Data.rtxData.innerAlpha);
    
                c.rtxShader.innerShadowAngle = stage_Data.rtxData.innerAngle;
                c.rtxShader.innerShadowDistance = stage_Data.rtxData.innerDistance;
                c.rtxShader.updateColorShift();

                if (stage_Data.rtxData.pointLight != null)
                {
                    c.rtxShader.pointLight = stage_Data.rtxData.pointLight;
                    c.rtxShader.lightX = stage_Data.rtxData.lightX;
                    c.rtxShader.lightY = stage_Data.rtxData.lightY;
    ...
    override public function new(?stageName:String, ?colorswap:ColorSwap = null) {
        super();
        if (colorswap == null)
            colorSwap = new ColorSwap();
        else 
            colorSwap = colorswap; //save hue shift between stage changes
